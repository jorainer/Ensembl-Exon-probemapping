...
microarray to all cDNAs and non-coding RNAs as well as to genomic DNA and re-annotation of these alignments to genes and transcripts (defined in Ensembl version \Sexpr{ENSEMBL_VERSION}).
...
save( annot, file=paste( "annotation/annot_huexon_transcriptlevel_",
               ENSEMBL_VERSION,
               ".Rdata",sep="" ) )
