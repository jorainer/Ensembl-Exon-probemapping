...
for the Affymetrix Mouse Gene ST microarray. The CDF file/package
bases on a complete re-alignment of all probe sequences on the corresponding
microarray to all cDNAs and non-coding RNAs as well as to genomic DNA and re-annotation of these alignments to
...
probes targeting the specific transcript. Transcript probe sets with
the same set of probes are merged to a single probe set.
...
\begin{scriptsize}
<<>>=
Info <- readLines( paste( "CDF/mogene10stv1_",
                         ENSEMBL_VERSION,
...
\begin{scriptsize}
<<results=hide>>=
convertCdf(filename=paste( "CDF/mogene10stv1_",
             ENSEMBL_VERSION,
             "_030.cdf", sep="" ),
           outFilename=paste( "CDF/mogene10stv1_",
             ENSEMBL_VERSION
...
\begin{scriptsize}
<<results=hide>>=
CDFNAME <- paste( "mogene10stv1",
                 sub( ENSEMBL_VERSION, pattern="_", replacement="" ),
                 "cdf", sep="" )
make.cdf.package(filename=paste( "CDF/mogene10stv1_",
                   ENSEMBL_VERSION,
                   "_030-bin.cdf", sep="" ),
...
<<results=hide>>=
gc()
annot <- read.table( gzfile( paste(  "CDF/mogene10stv1_",
                                        ENSEMBL_VERSION
                                        ,"_030.cdf.annotation.txt.gz", sep="" ) ),
                         header=TRUE, as.is=TRUE, sep="\t" )
rownames( annot ) <- annot[ , "probeset_id" ]
save( annot, file=paste( "annotation/annot_mogene_transcriptlevel_",
               ENSEMBL_VERSION,
               ".Rdata",sep="" ) )
...
genes. \Sexpr{length(grep(annot$transcript_id,pattern="ENS"))} probesets target transcripts form the Ensembl database (each probeset defined in the CDF and annotation file  can target more than one transcript, thus the number of probesets is smaller than the number of transcripts that can be detected).
...
dummy <- read.table( gzfile( paste( "CDF/mogene10stv1_",
                                   ENSEMBL_VERSION,
