...
Creating a \textit{gene level} CDF (Cel Definition File) package for the Affymetrix Mouse Exon ST microarray. The CDF file/package bases on a complete re-alignment of all probe sequences on the corresponding microarray to all cDNAs and non-coding RNAs as well as to genomic DNA and re-annotation of these alignments to genes (defined in Ensembl version \Sexpr{ENSEMBL_VERSION}).

The CDF contains definitions of geme probe sets containing all probes targeting any of the gene's transcript.
...
save( annot, file=paste( "annotation/annot_moexon_genelevel_",
               ENSEMBL_VERSION,
               ".Rdata",sep="" ) )
