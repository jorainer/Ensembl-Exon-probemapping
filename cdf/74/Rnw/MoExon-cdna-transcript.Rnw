...
save( annot, file=paste( "annotation/annot_moexon_transcriptlevel_",
               ENSEMBL_VERSION,
               ".Rdata",sep="" ) )
...
At last some information about the number of genes/transcripts... The CDF contains definition for \Sexpr{(nrow(annot)-length(bgprobes))} different transcripts, which represent \Sexpr{length(unique(annot[-bgprobes,"gene_id"]))} Ensembl genes. \Sexpr{length(grep(annot$transcript_id,pattern="ENS"))} probesets target transcripts form the Ensembl database (each probeset defined in the CDF and annotation file can target more than one transcript, thus the number of probesets is smaller than the number of transcripts that can be detected).
